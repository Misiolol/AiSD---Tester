...
def generate_increasing_sequence(n):
    num_range = n * 10 
    return sorted(random.sample(range(1, num_range + 1), n))

def save_sequence_to_file(n):
    directory = "./"
    os.makedirs(directory, exist_ok=True)
    filename = os.path.join(directory, f"increasing_{n}.txt")
    sequence = generate_increasing_sequence(n)
    with open(filename, "w") as f:
        f.write("\n".join(map(str, sequence)) + "\n")
        f.write("k\n")
    print(f"Saved increasing sequence to {filename}")
